...
func TestDatabaseLoggerRemovesBoundValues(t *testing.T) {
	...
	statement, values := filter.ParamsFilter(
		context.Background(),
		"SELECT * FROM members WHERE password_hash = ?",
		"must-never-reach-logs",
	)
	if statement != "SELECT * FROM members WHERE password_hash = ?" || len(values) != 0 {
...
func TestRequestLoggingBoundaryDoesNotWriteBusinessQueryValues(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodGet, "/search?q=private-query&open_id=private-open-id&contains[name]=private-name", nil)
