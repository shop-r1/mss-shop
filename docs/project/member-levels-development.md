...
delete remain unavailable. The first cluster migration test projects the
already imported fixed-tenant slice into the final business schema and proves
that the projection is complete and isolated. It does not mutate a member
level and does not close `MEMBER-001`.
...
The verifier receives no Kubernetes API credential, runs a read-only database
transaction, emits no row values or connection material and is removed by its
TTL. This is data-projection evidence only, not mutation acceptance.
...
Immutable-image CI remains the publication gate. Cluster evidence must
additionally bind the reconciler, runtime and disposable verifier to the same
full Git revision and image digests.
